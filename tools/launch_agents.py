...
def register_instance(instance_id: str) -> Tuple[str, bool, str]:
    ...
    result = subprocess.run([PYTHON, str(script), instance_id, "--no-default"], capture_output=True, text=True)
...
def start_responder(instance_id: str) -> Tuple[str, bool, str]:
    ...
    try:
        proc = subprocess.Popen([PYTHON, str(script), instance_id], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
...
def parse_args(argv: Iterable[str]) -> argparse.Namespace:
    ...
    p.add_argument("--prefix", type=str, default="agent", help="Instance id prefix (default: agent)")
