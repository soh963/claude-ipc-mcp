...
def check_broker_connection() -> bool:
    ...
    try:
        with socket.create_connection((BROKER_HOST, BROKER_PORT), timeout=2):
            return print_check("Broker Connection", True, f"Successfully connected to {BROKER_HOST}:{BROKER_PORT}")
...
def check_session_file() -> dict | None:
    ...
    try:
        ...
        if "instance_id" in session_data and "session_token" in session_data:
            print_check("Session File", True, f"Valid JSON for instance '{session_data['instance_id']}'")
            return session_data
        else:
...
def check_shared_secret() -> bool:
    ...
    if secret:
        ...
        return print_check("Shared Secret Env Var", False, "IPC_SHARED_SECRET is not set (Note: Only an issue if server requires it)")
...
def check_instance_registration(session_data: dict | None) -> bool:
    ...
    try:
        ...
        if f"ID: {instance_id}" in result.stdout:
            return print_check("Instance Registration", True, f"'{instance_id}' is actively registered")
        else:
            return print_check("Instance Registration", False, f"'{instance_id}' not found in active list")
    except (subprocess.CalledProcessError, subprocess.TimeoutExpired) as e:
        error_output = e.stderr or e.stdout or str(e)
        return print_check("Instance Registration", False, f"ipc_list.py failed: {error_output.strip()}")
...
def main():
    ...
    if not broker_ok:
        recommendations.append("The IPC message broker is not running. Start it with `claude-ipc-mcp`.")
    if not session_data:
        recommendations.append("Your session file is missing or corrupt. Register your instance with `python tools/ipc_register.py <your_id>`.")
    if not schema_ok:
        recommendations.append("The database schema is incorrect or corrupt. This is a critical error. The database may need to be rebuilt or the code updated to match the schema.")

    # Final Diagnosis
    if not recommendations:
        print("✅ No major issues found. Your configuration appears to be correct.")
        print("If you still have issues, check for rate limiting or specific logic errors in your AI.")
    else:
