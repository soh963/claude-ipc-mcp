...
# Default broker endpoint; can be overridden via environment for tests
BROKER_HOST = os.environ.get("IPC_BROKER_HOST", "127.0.0.1")
try:
    BROKER_PORT = int(os.environ.get("IPC_BROKER_PORT", "9876"))
except Exception:
    BROKER_PORT = 9876
...
def register_instance(
    instance_id: str, *, set_default: bool = True
    ...
    try:
        host = os.environ.get("IPC_BROKER_HOST", BROKER_HOST)
        try:
            port = int(os.environ.get("IPC_BROKER_PORT", str(BROKER_PORT)))
        except Exception:
            port = BROKER_PORT
        ...
        return False, f"Cannot connect to broker at {host}:{port} ({exc}).", None
...
def parse_args(argv: Iterable[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Register an IPC instance and optionally start an auto-responder."
    )
...
def main(argv: Iterable[str]) -> int:
    ...
    success, message, session_path = register_instance(
        args.instance_id, set_default=not args.no_default
    )
    if not success:
    ...
    print("  Send: python tools/ipc_send.py --instance " f'{args.instance_id} <to_id> "message"')
    print("  Check: python tools/ipc_check.py --instance " f"{args.instance_id}")
    print("=" * 50 + "\n")
    return 0
