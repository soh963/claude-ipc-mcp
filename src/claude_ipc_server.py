...
class RateLimiter:
    ...
    def is_allowed(self, instance_id: str) -> bool:
        ...
        with self.lock:
            ...
            self.requests[instance_id] = [
                ts for ts in self.requests[instance_id] if now - ts < self.window_seconds
            ]
...
class MessageBroker:
    ...
    def _load_from_database(self):
        ...
        try:
            ...
            conn.close()
            logger.info(
                f"Loaded {sum(len(q) for q in self.queues.values())} messages from database"
            )
        except Exception as e:
        ...
        with self.lock:
            ...
            if action == "register":
                ...
                if not self._validate_instance_id(instance_id):
                    return {
                        "status": "error",
                        "message": "Invalid instance ID format. Use 1-32 alphanumeric characters, hyphens, or underscores.",
                    }
                ...
                if shared_secret:
                    ...
                    expected_token = hashlib.sha256(
                        f"{instance_id}:{shared_secret}".encode()
                    ).hexdigest()
                ...
                if queued_count > 0:
                    response["message"] = (
                        f"Registered {instance_id} with {queued_count} queued messages"
                    )
                ...
                if len(self.queues[resolved_to]) >= 100:
                    return {
                        "status": "error",
                        "message": f"Message queue full for {resolved_to} (100 message limit)",
                    }
                ...
                if forwarded:
                    return {
                        "status": "ok",
                        "message": f"Message forwarded from {to_id} to {resolved_to}",
                    }
                elif future_delivery:
                    return {
                        "status": "ok",
                        "message": f"Message queued for {resolved_to} (not yet registered)",
                    }
                else:
                ...
                if old_id in self.last_rename:
                    ...
                    if time_since_last < 3600:  # 1 hour in seconds
                        ...
                        return {
                            "status": "error",
                            "message": f"Rate limit: can rename again in {minutes_left} minutes",
                        }
    ...
    return [
        ...
        Tool(
            ...
            inputSchema={
                "type": "object",
                "properties": {
                    "from_id": {"type": "string", "description": "Your instance ID"},
                    "to_id": {"type": "string", "description": "Target instance ID"},
                    "content": {"type": "string", "description": "Message content"},
                    "data": {"type": "object", "description": "Optional structured data to send"},
                },
        ...
        Tool(
            ...
            inputSchema={
                "type": "object",
                "properties": {
                    "old_id": {"type": "string", "description": "Your current instance ID"},
                    "new_id": {"type": "string", "description": "The new instance ID you want"},
                },
...
async def call_tool(name: str, arguments: Dict[str, Any]) -> List[TextContent]:
    ...
    if name == "register":
        ...
        response = BrokerClient.send_request(
            {"action": "register", "instance_id": instance_id, "auth_token": auth_token}
        )
        ...
        message = {"content": arguments["content"], "data": arguments.get("data", {})}
        response = BrokerClient.send_request(
            {
                "action": "send",
        ...
        try:
            ...
            return [
                TextContent(
                    type="text", text=f"Command output shared: {json.dumps(response, indent=2)}"
                )
            ]
        ...
        check_response = BrokerClient.send_request(
            {"action": "check", "instance_id": instance_id, "session_token": current_session_token}
        )

        if check_response.get("status") != "ok":
            return [
                TextContent(
                    type="text", text=f"Error checking messages: {check_response.get('message')}"
                )
            ]
