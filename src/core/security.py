...
class TLSManager:
    ...
    def _generate_self_signed_cert(self):
        ...
        subprocess.run([
            ...
            '-out', str(self.cert_path), '-days', '365',
            '-subj', '/CN=localhost/O=IPC/C=US'
        ], check=False)
...
class AuditLogger:
    ...
    def log_authorization(self, user: str, resource: str, action: str, granted: bool):
        ...
        self.log_event(
            'authorization',
            user,
            {'resource': resource, 'action': action, 'granted': granted}
        )
...
class SecurityManager:
    ...
    def __init__(self, config: SecurityConfig = None):
        ...
        self.tls_manager = TLSManager(self.config.tls_cert_path, self.config.tls_key_path) if self.config.enable_tls else None
